...
=======
  const sourceIp = String(req.ip || req.socket.remoteAddress || '')
    .replace(/^::ffff:/, '');
  ...
  const accessDenied =
    ...
    (config.nodeEnv === 'production' &&
      !config.onebill.allowedIps.includes(sourceIp));

  if (accessDenied) {
    // Return the response shape expected by the requested transaction.
    if (req.path.endsWith('/BillPayment')) {
      return res.status(401).json({
        response_Code: '04',
        Identification_parameter: '',
...
>>>>>>> 5b8f895 (Update production backend configuration)
    return res.status(401).json({
      response_Code: '04',
