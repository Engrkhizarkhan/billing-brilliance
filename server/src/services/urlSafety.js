...
const isPrivateIp = (address) => {
  ...
  return !/^[23][0-9a-f]{3}:/.test(normalized) || normalized.startsWith('2001:db8:') || normalized.startsWith('2002:');
};

const resolveSafePublicHttpsUrl = async (value) => {
  let parsed;
  try {
...
const assertSafePublicHttpsUrl = async value => (await resolveSafePublicHttpsUrl(value)).url;
module.exports = { assertSafePublicHttpsUrl, resolveSafePublicHttpsUrl, isPrivateIp };
